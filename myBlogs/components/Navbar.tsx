  ...
  return (
    <>
      ...
      <header className={`hidden md:block w-full fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b ${showNav ? 'translate-y-0' : '-translate-y-full'} bg-white/40 dark:bg-slate-900/50 backdrop-blur-xl border-white/20 dark:border-white/5 shadow-sm`}>
        <div className="w-[90%] max-w-6xl mx-auto h-16 flex items-center gap-4 px-4 sm:px-[30px] box-border">
          <Link href="/" className="shrink-0 text-xl font-black text-slate-800 dark:text-white tracking-tighter hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-300">
          ...
          <div className="hidden md:flex shrink-0 items-center rounded-full border border-white/30 dark:border-white/10 bg-white/25 dark:bg-slate-800/35 px-3 py-1.5 text-xs font-bold text-slate-600 dark:text-slate-300 shadow-sm backdrop-blur-md">
            <VisitorCounter />
          </div>
          <nav className="ml-auto flex gap-5 xl:gap-8 text-sm font-bold whitespace-nowrap">
            {/* PC端依然使用全量的 navLinks */}
            {navLinks.map((link) => {
