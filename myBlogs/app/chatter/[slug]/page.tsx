    ...
    <div className="min-h-screen relative pb-20">
      ...
      <PageTransition>
        <main className="w-[95%] md:w-[90%] max-w-6xl mx-auto mt-24 md:mt-28 flex flex-col lg:flex-row gap-6 md:gap-8 relative z-10">
          ...
          <article className="flex-1 relative bg-white/60 dark:bg-slate-800/50 backdrop-blur-xl rounded-[40px] shadow-2xl border border-white/40 dark:border-white/10 overflow-hidden transition-colors duration-700">
            {chatterData.cover && (
              <div className="w-full aspect-video bg-slate-200 dark:bg-slate-700 relative group">
                ...
                {/* 🌟 封面右下角：浏览次数（单个用户当天浏览 +1） */}
                <ViewsBadge kind="chatter" slug={chatterData.slug} countOnMount label="浏览" className="absolute bottom-3 right-3 md:bottom-4 md:right-4" />
              </div>
            )}

            {!chatterData.cover && (
              <ViewsBadge kind="chatter" slug={chatterData.slug} countOnMount label="浏览" className="absolute top-4 right-4 md:top-6 md:right-6 z-20" />
            )}
