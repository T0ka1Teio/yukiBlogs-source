...
import ViewsBadge, { ViewsBadgeHandle } from '../../components/ViewsBadge';
...
export default function MomentList({ moments, authorName, avatarUrl }: any) {
  ...
  const viewsRefs = useRef<Record<string, ViewsBadgeHandle | null>>({});
  ...
  const renderImages = (moment: any) => {
    const images: string[] = moment.images;
    ...
    if (count === 1) {
      return (
        <div className="mt-4 md:mt-8 flex justify-start sm:justify-center w-full">
          <div onClick={() => { setLightbox({ images, index: 0 }); viewsRefs.current[moment.id]?.trigger(); }} className="max-w-[80%] sm:max-w-[280px] overflow-hidden rounded-xl md:rounded-2xl border border-slate-200/50 dark:border-white/10 shadow-lg md:shadow-xl cursor-zoom-in group">
      ...
      <div className="w-full flex justify-start sm:justify-center mt-4 md:mt-8">
        <div className="grid gap-1.5 md:gap-2 sm:mx-auto" style={{ gridTemplateColumns: `repeat(${columns}, 1fr)`, width: '100%', maxWidth: maxWidth }}>
          {images.slice(0, 9).map((src, idx) => {
            ...
            return (
              <div key={idx} onClick={() => { setLightbox({ images, index: idx }); viewsRefs.current[moment.id]?.trigger(); }} className="group relative aspect-square overflow-hidden rounded-lg md:rounded-xl bg-slate-200/20 dark:bg-slate-700/20 border border-slate-200/50 dark:border-white/10 cursor-zoom-in">
  ...
  const renderMomentCard = (moment: any) => (
    ...
    >
      {/* 🌟 卡片右上角：浏览次数 */}
      <ViewsBadge
        ref={(el) => { viewsRefs.current[moment.id] = el; }}
        kind="moment"
        slug={moment.id}
      ...
      <div className="mt-5 md:mt-10 flex items-center justify-between">
        ...
        <button onClick={() => {
          const opening = openCommentId !== moment.id;
          setOpenCommentId(opening ? moment.id : null);
          if (opening) viewsRefs.current[moment.id]?.trigger();
        }} className={`w-8 h-8 md:w-10 md:h-10 flex items-center justify-center shrink-0 rounded-full transition-all shadow-sm ${openCommentId === moment.id ? 'bg-indigo-500 text-white shadow-indigo-500/30 rotate-12' : 'bg-white/80 dark:bg-slate-800 text-slate-400 hover:text-indigo-500 hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
