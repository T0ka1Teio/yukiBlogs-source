...
- 同步 Manager 与博客前端的最新脱敏源码，加入多音源搜索、可用性过滤、试听和流式播放。
- 完善云端图床批量上传、背景显示区域裁剪、编辑器图片与表格交互、草稿同步及文章更新时间展示。
- 优化 Manager 窗口和拖拽交互，并加强运行数据、私人配置与内容目录的公开仓库排除规则。
- 发布前安全扫描、类型检查、Lint、59 项自动化测试及双项目生产构建均已通过；详情见 [更新日志](UpdateLog.md)。
