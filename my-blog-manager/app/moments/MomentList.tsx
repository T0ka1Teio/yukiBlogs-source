...
import { MapPin, MessageSquare, Clock, Sparkles, Search, ArrowDownAZ, ArrowUpZA, ChevronLeft, ChevronRight, Ghost, Plus, X, Send, Link as LinkIcon, Zap, Trash2, AlertTriangle } from 'lucide-react';
import MomentComments from '../../components/MomentComments';
import PhotoWallPicker from '../../components/PhotoWallPicker';
...
export default function MomentList({ moments, authorName, avatarUrl }: any) {
  ...
  const handleTogglePhotoWallImage = (url: string) => {
    setNewMoment((prev) => ({
      ...prev,
      images: prev.images.includes(url)
        ? prev.images.filter((image) => image !== url)
        : [...prev.images, url],
    }));
  };
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex flex-col gap-4 mb-6">
                <PhotoWallPicker
                  selectedUrls={newMoment.images}
                  onToggle={handleTogglePhotoWallImage}
                  multiple
                />
