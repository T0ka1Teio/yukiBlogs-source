...
import { AlertTriangle, Search, Trash2, X, Sparkles, Pencil, FileUp, Loader2 } from 'lucide-react';
...
function DraftsContent() {
  ...
  const handleImportMarkdown = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.toLowerCase().endsWith('.md')) {
      showToast("请选择 .md 格式的 Markdown 文件", "warning");
      input.value = "";
      return;
    }
    ...
    try {
      const configRes = await fetch(`/backend_config.json?t=${Date.now()}`);
      if (!configRes.ok) throw new Error("读取后端配置失败");
      const configData = await configRes.json();

      const uploadData = new FormData();
      uploadData.append("file", file);
      const res = await fetch(`http://127.0.0.1:${configData.api_port}/api/drafts/import`, {
        method: "POST",
        body: uploadData,
      });
      const data = await res.json();
        ...
        <main className="w-[90%] max-w-5xl mx-auto mt-28 relative z-10">
          ...
          <div className="mb-8 flex flex-wrap gap-4">
            ...
            <button
              ...
              title="支持 YAML Frontmatter；无标题时读取一级标题或文件名"
              className="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 disabled:cursor-wait disabled:opacity-60 text-white rounded-2xl font-black text-sm shadow-lg shadow-emerald-500/30 transition-all active:scale-95 flex items-center gap-2"
            >
