...
import { useRuntimeSiteConfig } from '../../components/RuntimeSiteConfigProvider';
...
export default function AlchemyLab({ posts = [], chatters = [], moments = [] }: any) {
  ...
  const [runtimeAlbums, setRuntimeAlbums] = useState(bundledAlbums);
  const [runtimeFriends, setRuntimeFriends] = useState(bundledFriends);
  useEffect(() => { setMounted(true); }, []);
  useEffect(() => {
    let cancelled = false;
    loadRuntimeContent().then(content => {
      if (!cancelled) {
        setRuntimeAlbums(content.albums);
        setRuntimeFriends(content.friends);
      }
    }).catch(() => {});
  ...
  const rpgStats = useMemo(() => {
    ...
    // 统计照片与友链 (纯发徽章，不加经验)
    const totalPhotos = runtimeAlbums.reduce((acc: number, curr: any) => acc + (curr.photos?.length || 0), 0);
    const totalFriends = runtimeFriends.length;
  ...
  }, [posts, chatters, moments, runtimeAlbums, runtimeFriends]);
