...
// 🌟 核心修复：与归档页一致，强制实时读取文件系统并用 YUKIBLOGS_CONTENT_ROOT
// 定位内容目录（打包运行时 process.cwd() 指向 .next/standalone）。
...
const CONTENT_DIRECTORIES = {
  posts: path.join(/* turbopackIgnore: true */ contentRoot, 'posts'),
  chatters: path.join(/* turbopackIgnore: true */ contentRoot, 'chatters'),
  moments: path.join(/* turbopackIgnore: true */ contentRoot, 'moments'),
} as const;

function getLocalItems(directoryName: keyof typeof CONTENT_DIRECTORIES, typeName: string) {
  const dirPath = CONTENT_DIRECTORIES[directoryName];
  let items: any[] = [];
  try {
