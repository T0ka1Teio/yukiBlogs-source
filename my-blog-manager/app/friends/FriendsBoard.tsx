...
export default function FriendsBoard() {
  ...
  useEffect(() => {
    ...
    const loadRuntimeState = async () => {
      try {
        const contentResponse = await loadRuntimeContent();
        if (!cancelled) setEditableFriends(contentResponse.friends);
      } catch {
        // Keep bundled data when the local runtime endpoints are unavailable.
      }
    };
    loadRuntimeState();
