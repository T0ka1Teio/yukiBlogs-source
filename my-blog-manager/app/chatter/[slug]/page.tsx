...
import { getPublicRuntimeSiteConfig } from '../../../lib/server/runtimeConfig';
...
async function getChatterData(slug: string, runtimeSiteConfig: typeof siteConfig) {
  const fullPath = path.join(contentRoot, 'chatters', `${slug}.md`);
  ...
  return {
    ...
    cover: data.cover || runtimeSiteConfig.defaultPostCover
  };
}

function getRecentChatters(currentSlug: string) {
  const chattersDirectory = path.join(contentRoot, 'chatters');
...
export default async function ChatterDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const runtimeSiteConfig = { ...siteConfig, ...getPublicRuntimeSiteConfig() } as typeof siteConfig;
  const chatterData = await getChatterData(resolvedParams.slug, runtimeSiteConfig);
          ...
          <aside className="w-full lg:w-[320px] flex flex-col gap-6 flex-shrink-0">
            <div className="bg-white/60 dark:bg-slate-800/50 backdrop-blur-xl rounded-3xl p-6 border border-white/40 dark:border-white/10 shadow-xl text-center">
              <div className="w-20 h-20 mx-auto rounded-full p-1 bg-gradient-to-tr from-indigo-500 to-purple-500 shadow-md mb-4 hover:rotate-3 transition-transform">
                <img src={runtimeSiteConfig.avatarUrl} alt="avatar" className="w-full h-full rounded-full object-cover bg-white" />
              </div>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">{runtimeSiteConfig.authorName}</h3>
              <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-medium mb-4">{runtimeSiteConfig.bio}</p>
              <ClientSocials />
            </div>
