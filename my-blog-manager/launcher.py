...
def configure_console_output():
    """Prevent Windows legacy consoles from crashing on Unicode status text."""
...
def get_free_port(excluded=None):
...
def get_frontend_port():
    port = int(os.environ.get('YUKIBLOGS_FRONTEND_PORT', '3210'))
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as probe:
        try:
            probe.bind(('127.0.0.1', port))
        except OSError as error:
            raise RuntimeError(
                f'固定前端端口 {port} 已被占用；请关闭旧控制台后重试，'
                ' 或设置 YUKIBLOGS_FRONTEND_PORT 并同步修改 GitHub OAuth Callback URL。'
            ) from error
    return port
...
def prepare_standalone_assets(base_dir=BASE_DIR):
    """Copy assets omitted by Next.js standalone output into its runtime tree."""
    standalone_dir = os.path.join(base_dir, '.next', 'standalone')
    if not os.path.isdir(standalone_dir):
        return False

    asset_pairs = [
        (os.path.join(base_dir, '.next', 'static'), os.path.join(standalone_dir, '.next', 'static')),
        (os.path.join(base_dir, 'public'), os.path.join(standalone_dir, 'public')),
    ]
...
if __name__ == "__main__":
    try:
        ...
        print(f">>> ❌ {error}")
        sys.exit(1)
    backend_port = get_free_port({frontend_port})

    env_vars = os.environ.copy()
    env_vars["PORT"] = str(frontend_port)
    env_vars["YUKIBLOGS_CONTENT_ROOT"] = EXE_DIR

    standalone_dir = os.path.join(BASE_DIR, '.next', 'standalone')
    server_js = os.path.join(standalone_dir, 'server.js')

    if os.path.exists(server_js):
        prepare_standalone_assets(BASE_DIR)
