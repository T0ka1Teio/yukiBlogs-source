...
import { ImageIcon, Tag, FileText, X, Clock, CloudUpload, Smile, Save, Send, Crop, Loader2 } from 'lucide-react';
...
export default function MetaMatrix({
  ...
  useEffect(() => {
    ...
    const filename = cover.slice('/uploads/covers/'.length);

    const resolveLocalCover = async () => {
      try {
        const configResponse = await fetch(`/backend_config.json?t=${Date.now()}`);
        if (!configResponse.ok) throw new Error('后端配置读取失败');
        const config = await configResponse.json();
        if (!cancelled) {
          setCoverPreviewUrl(`http://127.0.0.1:${config.api_port}/api/picbed/covers/${encodeURIComponent(filename)}?t=${Date.now()}`);
        }
      } catch {
        if (!cancelled) setCoverPreviewUrl('');
      } finally {
        if (!cancelled) setIsResolvingCover(false);
      }
    };

    void resolveLocalCover();
    return () => {
      cancelled = true;
    };
  }, [cover]);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden relative">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-8 flex flex-col gap-10">
        ...
        <div className="flex flex-col">
          ...
          <div onClick={onOpenImageTool} className="w-full aspect-video bg-black/10 dark:bg-black/40 rounded-[32px] border-2 border-dashed border-white/10 flex items-center justify-center overflow-hidden mb-4 group relative cursor-pointer shadow-inner transition-all">
            {coverPreviewUrl ? (
              <img src={coverPreviewUrl} alt="封面预览" className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
            ) : cover && isResolvingCover ? (
              <div className="flex flex-col items-center gap-2 text-slate-400">
                <Loader2 size={24} className="animate-spin text-indigo-400" />
                <span className="text-[9px] font-black uppercase tracking-widest">Loading Cover</span>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-2 text-slate-400">
            ...
            {cover && (
              <button
                ...
                className="absolute bottom-3 right-3 z-10 flex items-center gap-1.5 rounded-xl bg-indigo-500 px-3 py-2 text-[10px] font-black text-white shadow-lg shadow-indigo-950/30 transition hover:bg-indigo-400 sm:translate-y-2 sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100"
              >
                <Crop size={13} /> 裁剪封面
              </button>
            )}
      ...
      <CoverCropper
        isOpen={isCropOpen && Boolean(cover)}
        imageUrl={coverPreviewUrl || cover}
