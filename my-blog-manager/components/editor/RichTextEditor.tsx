...
const hasMarkdownBlockSyntax = (text: string) => {
  const blockSyntax = /^\s{0,3}(?:#{1,6}\s+\S|>\s+\S|(?:[-+*]|\d+[.)])\s+\S|```|~~~)/m;
  const gfmTable = /^\s*\|?.+\|.+\r?\n\s*\|?\s*:?-{3,}:?\s*(?:\|\s*:?-{3,}:?\s*)+\|?\s*$/m;

  return blockSyntax.test(text) || gfmTable.test(text);
};
  ...
  return (
    <div className="flex flex-col h-full w-full min-h-0 bg-transparent relative">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .editor-content-area .tableWrapper {
          margin: 1.75rem 0;
          overflow-x: auto;
          border-radius: 1rem;
        ...
        .dark .editor-content-area td {
          background: rgba(15, 23, 42, 0.35);
        }
        .editor-content-area th > p,
        .editor-content-area td > p {
          margin: 0 !important;
          font-size: 1rem !important;
          line-height: 1.6 !important;
        }
        .editor-content-area .selectedCell::after {
          position: absolute;
          inset: 0;
          z-index: 2;
          pointer-events: none;
        ...
        .editor-content-area .column-resize-handle {
          position: absolute;
          top: 0;
          right: -2px;
          bottom: 0;
      ...
      <div className="shrink-0 px-8 py-2.5 border-y border-white/20 dark:border-white/10 flex flex-wrap items-center gap-1.5 bg-white/10 dark:bg-black/20 backdrop-blur-md z-50">
        ...
        {editor.isActive('table') && (
          <div className="flex items-center gap-1 rounded-xl border border-indigo-500/20 bg-indigo-500/10 px-1.5 py-1">
            <Btn onClick={() => editor.chain().focus().addRowAfter().run()} title="在下方添加一行"><Rows3 size={15} /></Btn>
            <Btn onClick={() => editor.chain().focus().addColumnAfter().run()} title="在右侧添加一列"><Columns3 size={15} /></Btn>
            <Btn onClick={() => editor.chain().focus().deleteTable().run()} title="删除表格"><Trash2 size={15} /></Btn>
          </div>
        )}
