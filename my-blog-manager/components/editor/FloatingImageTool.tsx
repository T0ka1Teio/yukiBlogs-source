...
export default function FloatingImageTool({ isOpen, onClose, onInsert, sourceMode = 'upload' }: FloatingImageToolProps) {
  const { showToast } = useToast();
  const primaryTab = sourceMode === 'photowall' ? 'photowall' : 'upload';
  const [activeTab, setActiveTab] = useState<'upload' | 'photowall' | 'url'>(primaryTab);
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className={`${sourceMode === 'photowall' ? 'w-[420px] max-w-[92vw]' : 'w-80'} bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl rounded-[32px] shadow-2xl border border-white/50 dark:border-white/10 overflow-hidden flex flex-col cursor-move`}
        >
          {/* 标题栏 */}
          <div className="flex justify-between items-center p-5 border-b border-white/30 dark:border-slate-700/50 bg-white/50 dark:bg-slate-800/50">
            <h3 className="text-sm font-black text-slate-800 dark:text-slate-100 flex items-center gap-2">
              <span className="text-emerald-500 text-lg">{sourceMode === 'photowall' ? '🖼️' : '☁️'}</span>
          ...
          <div className="p-6 cursor-default bg-white/20 dark:bg-slate-900/20">
            ...
            {!uploadedUrl && (
              <div className="flex bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-2xl mb-5">
                <button
                  onClick={() => setActiveTab(primaryTab)}
                  className={`flex-1 py-2 text-xs font-bold rounded-xl transition-all ${activeTab === primaryTab ? 'bg-white dark:bg-slate-700 text-emerald-500 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  {sourceMode === 'photowall' ? '照片墙' : '云端上传'}
                </button>
                <button
