...
import { ShieldCheck, GitBranch, Save, Wand2, Key, Copy, Check, CloudUpload, Code } from 'lucide-react';
...
export default function RepoSection() {
  ...
  const [modalConfig, setModalConfig] = useState<{ isOpen: boolean; type: 'init' | 'upload' | 'ssh' | null }>({
    isOpen: false,
    type: null
  });

  const [sshConfig, setSshConfig] = useState<{ key: string }>({ key: "" });
  ...
  const handleGetSSH = async () => {
    try {
      const configRes = await fetch(`/backend_config.json?t=${Date.now()}`);
      const config = await configRes.json();
      const res = await fetch(`http://127.0.0.1:${config.api_port}/api/deploy/ssh/key?type=source`);
      const data = await res.json();
      if (data.success) {
        setSshConfig({ key: data.key });
  ...
  const executeUploadSource = async () => {
    setModalConfig({ isOpen: false, type: null });
    setIsUploading(true);
    showToast("☁️ 正在同步源码至 EdgeOne 触发仓库...", "info");
    try {
      const configRes = await fetch(`/backend_config.json?t=${Date.now()}`);
      const configData = await configRes.json();
      ...
      const data = await res.json();
      if (data.success) showToast("✅ 源码上传成功！EdgeOne 即将开始自动构建", "success");
    ...
    <>
      <motion.section initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl border border-white/50 dark:border-slate-800/50 rounded-[40px] p-8 shadow-2xl relative z-10">
        <div className="flex justify-between items-center mb-8">
            <h2 className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-2">🚀 EdgeOne 源码部署</h2>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/30 p-5 rounded-3xl border border-slate-100 dark:border-slate-700/50">
            ...
            <div className="p-4 bg-blue-500/5 border border-blue-500/20 rounded-2xl mb-6 relative">
              <div className="flex justify-between items-center mb-3">
                <h4 className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase flex items-center gap-1"><Code size={12}/> 源码托管库 (用于 EdgeOne 触发)</h4>
                <button onClick={(e) => { e.preventDefault(); handleGetSSH(); }} className="flex items-center gap-1 px-3 py-1.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-full text-[10px] font-black hover:bg-blue-500 hover:text-white transition-all">
                  <Key size={12} /> 获取源码库密匙
                </button>
              </div>
            ...
            <div className="pt-6 border-t border-slate-200 dark:border-slate-700/50 flex flex-col gap-3">
               <button onClick={() => setModalConfig({isOpen: true, type: 'init'})} disabled={isInitializing} className="w-full flex items-center justify-center gap-2 py-4 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800 rounded-2xl text-sm font-black shadow-sm active:scale-95 transition-all hover:bg-indigo-100 dark:hover:bg-indigo-900/50">
                  <Wand2 size={18} className={isInitializing ? "animate-spin" : ""} /> {isInitializing ? "初始化中..." : "🪄 初始化源码部署环境"}
               </button>

               <div className="flex gap-3 flex-col md:flex-row">
                 <button onClick={() => setModalConfig({isOpen: true, type: 'upload'})} disabled={isUploading} className="flex-1 flex items-center justify-center gap-2 py-4 bg-blue-500 text-white rounded-2xl text-sm font-black shadow-lg shadow-blue-500/30 active:scale-95 transition-all hover:bg-blue-600">
                    <CloudUpload size={18} className={isUploading ? "animate-pulse" : ""} /> {isUploading ? "同步中..." : "☁️ 仅同步源码 (EdgeOne)"}
                 </button>
               </div>
            </div>
          </div>
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-md bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-[40px] shadow-2xl border border-white/50 p-10 text-center">
                ...
                {modalConfig.type === 'init' && (
                    <>
                        ...
                        <p className="text-sm text-slate-500 mb-8 leading-relaxed">初始化 Git 并配置 EdgeOne 源码同步。</p>
                ...
                {modalConfig.type === 'upload' && (
                    <>
                        ...
                        <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">同步源码至 EdgeOne？</h3>
                        <p className="text-sm text-slate-500 mb-8 leading-relaxed">将本地源代码提交并推送到 GitHub 源码仓库，自动触发 EdgeOne 构建。</p>
                        <div className="flex gap-3">
                            <button onClick={() => setModalConfig({isOpen: false, type: null})} className="flex-1 py-4 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-2xl text-xs font-black transition-colors hover:bg-slate-200">取消</button>
