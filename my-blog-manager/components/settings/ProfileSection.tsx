          ...
          <div className="flex-1 w-full space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="col-span-1 md:col-span-2">
                ...
                <input type="text" value={safeData.title || ''} onChange={e => handleUpdate('title', e.target.value)} placeholder="例如: yukiBlogs" className="w-full bg-white/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl px-4 py-3 text-sm mt-1 outline-none font-bold focus:ring-2 focus:ring-indigo-500" />
              </div>

              <div className="col-span-1 md:col-span-2">
                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">站点主域名 (Site URL)</label>
                <input type="url" value={safeData.siteUrl || ''} onChange={e => handleUpdate('siteUrl', e.target.value)} placeholder="https://blog.example.com" className="w-full bg-white/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl px-4 py-3 text-sm mt-1 outline-none font-mono focus:ring-2 focus:ring-indigo-500" />
                <p className="mt-1 ml-1 text-[10px] text-slate-400">用于 canonical、Open Graph 与分享链接，请填写完整 HTTPS 地址且末尾不要加路径。</p>
              </div>
