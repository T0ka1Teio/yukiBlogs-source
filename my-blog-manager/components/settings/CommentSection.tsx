...
import { Save, Key, User, GitBranch, Shield, RefreshCw } from 'lucide-react';
...
export default function CommentSection({ formData, handleUpdate, pushToQueue }: CommentSectionProps) {
  ...
  const saveToQueue = () => {
    ...
    setOauthStatus('已加入队列；请点击右上角“更新本地”后再检测。');
  };

  const checkOAuth = async () => {
    setIsChecking(true);
    try {
      const response = await fetch(`/api/github?t=${Date.now()}`, { cache: 'no-store' });
      const data = await response.json() as { configured?: boolean; missing?: string[] };
      setOauthStatus(data.configured
        ? '✅ GitHub OAuth 必需配置已齐全'
        : `⚠️ 仍缺少：${(data.missing || []).join(', ') || '未知字段'}`);
    } catch {
      setOauthStatus('❌ OAuth 配置检测请求失败');
    } finally {
      setIsChecking(false);
    }
  };
      ...
      <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border border-white/50 dark:border-slate-800/50 rounded-[40px] p-8 shadow-xl">
        <div className="flex justify-between items-center mb-8 border-b border-white/30 dark:border-slate-700/50 pb-6">
          ...
          <div className="flex gap-3">
            <button onClick={checkOAuth} disabled={isChecking} className="px-5 py-3 bg-white/60 dark:bg-slate-800/60 rounded-2xl font-black text-sm flex items-center gap-2">
              <RefreshCw size={16} className={isChecking ? 'animate-spin' : ''} /> 检测授权配置
            </button>
            <button
              onClick={saveToQueue}
        ...
        <div className="space-y-6">
          {oauthStatus && <p className="rounded-2xl bg-indigo-500/10 px-4 py-3 text-sm font-bold text-indigo-600 dark:text-indigo-300">{oauthStatus}</p>}
          <p className="rounded-2xl bg-slate-500/10 px-4 py-3 text-xs font-bold text-slate-500 dark:text-slate-300">
            本地 GitHub OAuth Callback URL：<code className="ml-2">http://127.0.0.1:3210/</code>；线上请使用你的 EdgeOne 自定义域名。
          </p>
