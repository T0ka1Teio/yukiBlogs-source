...
export default function SiteDashboard() {
  ...
  const [buildDate, setBuildDate] = useState(bundledSiteConfig.buildDate || '2026-03-23T00:00:00');
  const [footerBadges, setFooterBadges] = useState<FooterBadge[]>(bundledSiteConfig.footerBadges || []);

  const parsedStartDate = new Date(buildDate).getTime();
  const startDate = Number.isFinite(parsedStartDate)
    ? parsedStartDate
    : new Date('2026-03-23T00:00:00').getTime();

  useEffect(() => {
    let cancelled = false;
    const loadRuntimeBuildDate = async () => {
      try {
        ...
        if (!cancelled && runtimeConfig.buildDate && Number.isFinite(Date.parse(runtimeConfig.buildDate))) {
          setBuildDate(runtimeConfig.buildDate);
        }
        if (!cancelled && Array.isArray(runtimeConfig.footerBadges)) {
          setFooterBadges(runtimeConfig.footerBadges);
        }
      } catch {
        // Keep the bundled value when the local runtime API is unavailable.
      }
    };
    loadRuntimeBuildDate();
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      // 计算运行时间
      const diff = now.getTime() - startDate;
