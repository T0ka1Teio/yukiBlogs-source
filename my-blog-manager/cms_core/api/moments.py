...
from cms_core.api.sync import delete_moment_from_configured_blog
...
def delete_moment(payload: DeletePayload):
    try:
        ...
        if os.path.exists(file_path):
            ...
            synced, sync_message = delete_moment_from_configured_blog(payload.id)
            message = "文件已删除"
            if synced:
                message += "，并已镜像到前端源码"
            else:
                message += f"；前端镜像未完成: {sync_message}"
